...
class AdytonGLViewport(QOpenGLWidget):
    ...
    def _draw_starfield(self, projection: QMatrix4x4, view: QMatrix4x4):
        if not self.star_points:
            return
        from OpenGL.GL import glDisable, glEnable, glBegin, glEnd, glColor4f, glVertex3f, GL_POINTS, GL_DEPTH_TEST, GL_BLEND, glBlendFunc, GL_SRC_ALPHA, GL_ONE

        glDisable(GL_DEPTH_TEST)
        glEnable(GL_BLEND)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE)

        # Align +Z to Galactic Center already handled by Venus wall orientation; no extra rotation
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        glMultMatrixf(projection.data())
        ...
        for sp in self.star_points:
            r, g, b = sp.color
            alpha = max(0.3, min(1.0, 1.2 - (sp.magnitude * 0.15 if sp.magnitude >= 0 else 0.2)))
            glColor4f(r, g, b, alpha)
            x, y, z = sp.direction
            glVertex3f(x * 2000.0, y * 2000.0, z * 2000.0)
        glEnd()
