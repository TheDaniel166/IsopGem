...
from PyQt6.QtGui import QFont, QPixmap, QImage, QClipboard, QGuiApplication, QShortcut, QKeySequence, QCursor, QAction, QTextCursor
...
from shared.ui.rich_text_editor.code_editor_widget import CodeEditorWidget
...
from .flowchart_builder_panel import FlowchartBuilderPanel
...
class MermaidEditorDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.builder_toggle_btn = QPushButton(qta.icon("fa5s.tools", color=COLORS["magus"]), "")
        self.builder_toggle_btn.setFixedSize(32, 32)
        self.builder_toggle_btn.setToolTip("Toggle Visual Builder (Point-and-Click)")
        self.builder_toggle_btn.setCheckable(True)
        self.builder_toggle_btn.setChecked(False)  # Hidden by default
        ...
        toolbar.addWidget(self.builder_toggle_btn)
        ...
        # Left: Visual Builder Panel (initially hidden)
        self.builder_panel = FlowchartBuilderPanel()
        self.builder_panel.code_changed.connect(self._on_builder_code_changed)
        self.builder_panel.hide()  # Hidden by default
        ...
        self.preview_view.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.preview_view.customContextMenuRequested.connect(self._show_preview_context_menu)
        ...
        splitter.addWidget(self.builder_panel)
        splitter.addWidget(editor_frame)
        splitter.addWidget(preview_frame)
        splitter.setSizes([0, 500, 500])  # Builder hidden initially
        
        self.main_splitter = splitter  # Store reference for toggling
    ...
    def _toggle_builder_panel(self, checked: bool):
        """Toggle the visual builder panel visibility."""
        if checked:
            # Show builder panel
            self.builder_panel.show()
            
            # Adjust splitter sizes to give builder panel space
            current_sizes = self.main_splitter.sizes()
            total_width = sum(current_sizes)
            
            # Give 25% to builder, split rest between editor and preview
            builder_width = int(total_width * 0.25)
            editor_width = int(total_width * 0.375)
            preview_width = total_width - builder_width - editor_width
            
            self.main_splitter.setSizes([builder_width, editor_width, preview_width])
            
            # Sync current code to builder
            code = self.code_editor.toPlainText().strip()
            if code and (code.startswith('flowchart') or code.startswith('graph')):
                try:
                    self.builder_panel.load_code(code)
                except Exception as e:
                    logger.warning(f"Could not load code into builder: {e}")
        else:
            # Hide builder panel
            self.builder_panel.hide()
            
            # Adjust splitter to give space back to editor and preview
            current_sizes = self.main_splitter.sizes()
            total_width = sum(current_sizes)
            
            editor_width = int(total_width * 0.5)
            preview_width = total_width - editor_width
            
            self.main_splitter.setSizes([0, editor_width, preview_width])
    ...
    def _on_template_selected(self, template_name: str):
        ...
        if template_name in MERMAID_TEMPLATES:
            ...
            if self.builder_toggle_btn.isChecked():
                try:
                    self.builder_panel.load_code(MERMAID_TEMPLATES[template_name])
                except Exception as e:
                    logger.warning(f"Could not sync template to builder: {e}")
    ...
    def _on_text_changed(self):
        ...
        # If builder is open and we're not updating from builder, try to sync
        if self.builder_toggle_btn.isChecked() and not self._updating_from_builder:
            code = self.code_editor.toPlainText().strip()
            if code and (code.startswith('flowchart') or code.startswith('graph')):
                # Debounce builder sync as well (don't sync on every keystroke)
                # This is intentionally "best effort" - if parsing fails, no big deal
                pass  # For now, we only sync when builder makes changes
    ...
    # === PHASE 5: POLISH & CONTEXT MENU ===
    
    def _show_preview_context_menu(self, pos):
        """Show context menu on preview panel with quick actions."""
        menu = QMenu(self)
        
        # Check if builder is active and has nodes
        if self.builder_toggle_btn.isChecked() and self.builder_panel.ast.nodes:
            # Add "Edit Node" actions for each node
            edit_menu = menu.addMenu(qta.icon("fa5s.edit", color=COLORS["stone"]), "Edit Node")
            
            for node in self.builder_panel.ast.nodes:
                node_label = f"{node.id}: {node.label[:20]}..." if len(node.label) > 20 else f"{node.id}: {node.label}"
                node_action = QAction(node_label, self)
                node_action.triggered.connect(lambda checked, nid=node.id: self._quick_edit_node(nid))
                edit_menu.addAction(node_action)
            
            menu.addSeparator()
        
        # Always available actions
        refresh_action = QAction(qta.icon("fa5s.sync-alt", color=COLORS["accent"]), "Refresh Preview", self)
        refresh_action.triggered.connect(self._render_preview)
        menu.addAction(refresh_action)
        
        copy_action = QAction(qta.icon("fa5s.clipboard", color=COLORS["scribe"]), "Copy to Clipboard", self)
        copy_action.triggered.connect(self._copy_to_clipboard)
        menu.addAction(copy_action)
        
        menu.addSeparator()
        
        save_png_action = QAction(qta.icon("fa5s.image", color=COLORS["seeker"]), "Save as PNG", self)
        save_png_action.triggered.connect(self._save_as_png)
        menu.addAction(save_png_action)
        
        save_svg_action = QAction(qta.icon("fa5s.vector-square", color=COLORS["magus"]), "Save as SVG", self)
        save_svg_action.triggered.connect(self._save_as_svg)
        menu.addAction(save_svg_action)
        
        # Show builder toggle if not visible
        if not self.builder_toggle_btn.isChecked():
            menu.addSeparator()
            show_builder_action = QAction(qta.icon("fa5s.tools", color=COLORS["magus"]), "Show Visual Builder", self)
            show_builder_action.triggered.connect(lambda: self.builder_toggle_btn.setChecked(True))
            menu.addAction(show_builder_action)
        
        menu.exec(self.preview_view.mapToGlobal(pos))
    
    def _quick_edit_node(self, node_id: str):
        """Quick edit node from context menu."""
        # Ensure builder is visible
        if not self.builder_toggle_btn.isChecked():
            self.builder_toggle_btn.setChecked(True)
        
        # Select the node in builder's dropdown
        index = self.builder_panel.node_list_combo.findText(node_id)
        if index >= 0:
            self.builder_panel.node_list_combo.setCurrentIndex(index)
            # Trigger the selection handler to populate edit fields
            self.builder_panel._on_node_selected(node_id)
            # Focus the label input for easy editing
            self.builder_panel.node_label_input.setFocus()
            self.builder_panel.node_label_input.selectAll()
