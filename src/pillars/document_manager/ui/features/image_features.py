...
class ImageInsertFeature(EditorFeature):
    """
    ...
    def __init__(self, parent_editor: 'RichTextEditor', gateway: Optional["ImageGateway"] = None):
        ...
        if gateway:
            ...
            saver = getattr(parent_editor, 'resource_saver', None)
            provider = getattr(parent_editor, 'resource_provider', None)
            
            # If not on wrapper, check inner editor (common pattern)
            if not provider and hasattr(parent_editor, 'editor'):
                 provider = getattr(parent_editor.editor, 'resource_provider', None)
            
            self.gateway = ImageGateway(parent_editor.editor, resource_saver=saver, resource_provider=provider)
...
class ImageEditFeature(EditorFeature):
    """
