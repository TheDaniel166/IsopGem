...
## Update Summary
**Changes Made**
- Added comprehensive documentation for the new Holy Book Teacher feature and its integration with the verse parsing system
- Documented the enhanced Gematria Calculator Window with multi-system comparison capabilities
- Updated architecture diagrams to reflect the expanded service layer with new calculator types
- Added detailed API reference materials for the Strategy Pattern implementation
- Enhanced documentation with data flow patterns and integration points for external systems
- Updated UI component analysis to include new window types and their specialized features
...
Hub[gematria_hub.py]
Calculator[gematria_calculator_window.py]
HolyBook[holy_book_teacher_window.py]
Batch[batch_calculator_window.py]
...
Batch --> BatchIO
Batch --> CalculationService
HolyBook --> VerseParser
end
```
...
+_setup_ui()
+_create_tool_card(icon : str, title : str, desc : str, color : str, callback)
+_open_calculator()
...
+__init__(calculators : List[GematriaCalculator], window_manager : Optional[WindowManager], parent : Optional[QWidget])
+_setup_ui()
+_select_calculator(system_name : str)
+_toggle_keyboard()
+_calculate()
+_calculate_all_methods(text : str, language : str)
+_save_calculation()
+_send_to_tablet()
+_on_total_context_menu(value : int, pos : QPoint)
}
...
+_update_preview()
+_process_batch()
+_on_progress_updated(current : int, total : int)
+_on_calculation_completed(text : str, status : str)
+_on_processing_finished(success_count : int, error_count : int)
}
...
GematriaHub --> BatchCalculatorWindow : "opens"
...
9. **Holy Book Teacher window not loading**: Verify that the document exists in the document manager and that the verse teacher service is properly initialized. Check the console for any initialization errors.

10. **Batch calculator not processing files**: Ensure that the CSV/Excel file contains the required columns (word/text, tags, notes) and that the file encoding is supported. Check the file format compatibility with the batch IO service.
