...
<!-- VISUAL: autogenerated from src/pillars/correspondences/services/formula_engine.py -->
```mermaid
classDiagram
    ArgumentMetadata <|-- FormulaEngine
    FormulaEngine : +evaluate(formula: str)
    FormulaEngine --> FormulaParser : uses
    FormulaParser : +parse()
```
